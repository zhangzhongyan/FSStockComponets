...
+ (void)calMACDWithData:(NSArray<JMKlineModel *>*)datas params:(NSArray *)params {
    ...
    [datas enumerateObjectsUsingBlock:^(JMKlineModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        JMMACDModel *m = [JMMACDModel new];
        m.DIFF = diffs[idx];
        m.DEA = deas[idx];
        m.MACD = bars[idx];
        obj.MACD = m;
...
+ (void)calKDJWithData:(NSArray<JMKlineModel *>*)datas params:(NSArray *)params {
    ...
    NSMutableArray<JMKlineModel*> *nineDaysdatas = @[].mutableCopy;
    ...
    for (int i = 0; i < datas.count; i ++) {
        ...
        double max = 0;
        double min = INFINITY;
        for (JMKlineModel *mn in nineDaysdatas) {
